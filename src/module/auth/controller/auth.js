...
export const signUp = asyncHandler(async (req, res, next) => {
    ...
    const user = await userModel.create({ fristName, lastName, userName, email, password: hash })
...
export const login = asyncHandler(async (req, res, next) => {
    ...
    if (user && bcrypt.compareSync(password, user.password)) {
        const token = jwt.sign({ id: user._id, email: user.email, userName: user.userName, role: user.role }
            , process.env.TOKEN_SIGNTURE, { expiresIn: 60 * 30 })
...
// res.send("<a href='link of signup page from El Front end' >Not Rigister user please go to Rigister and cheack you Email </a>")
